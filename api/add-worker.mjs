...
export default function addWorkerHandler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const { username, password, worker } = req.body || {};

  if (!username || !password || ADMIN_USERS[username] !== password) {
  ...
  try {
    if (fs.existsSync(WORKERS_FILE)) {
      workers = JSON.parse(fs.readFileSync(WORKERS_FILE, 'utf-8'));
    }
  } catch (e) {
    workers = [];
  }
